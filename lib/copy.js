...
function promptCallback (args) {
    ...
    var widgetsArr = args.widget.split(',');
    for (var i = 0, len = widgetsArr.length; i < len; i++) {
        widgetsArr[i] = widgetsArr[i].trim();
        aspfe.checkCWDPath(
        ...
        bizParams.widgetsList.push({
            name: widgetsArr[i],
            fullPath: bizParams.widgetsPath + path.sep + widgetsArr[i]
        });
    }
